...
- **CDP connection drops on long-running scripts.** browser-harness exits with `RuntimeError: fatal: CDP WS handshake failed: timed out during opening handshake` and no log is written. Fix: click Allow in the Chrome window (`chrome://inspect`), then re-run. Check Chrome for the popup whenever browser-harness returns exit code 1 immediately with no output.
...
The following domain skills are stored in this skills-repo rather than the upstream browser-harness repo:

- `domain-skills/chatgpt-learn/scraping.md` — learn.chatgpt.com JS-SPA, selector `main article`, discovery via `nav a`/`aside a`, URL patterns, 84 pages scraped 2026-07-11
